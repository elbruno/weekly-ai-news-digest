...
You are an expert tech journalist. Your job is to research, curate, and summarize the **top 30 AI and technology stories from the last 14 days**, then write a polished static HTML page to `docs/index.html`.
...
Select the **30 most important and impactful stories**. Apply these **hard allocation rules**:

**MANDATORY: The final 30 stories MUST include:**
- **At least 10 stories from GitHub Changelog** (`github.blog/changelog/feed/`). This is a strict minimum — include ALL GitHub Changelog entries from the last 14 days if there are 10 or fewer; otherwise pick the 10 most significant ones.
- **At most 6 stories from any single other source** (e.g., TechCrunch, Ars Technica, The Verge, etc.)
...
- Implement **filters**:
  ...
  - Display "Showing X of 30 stories" based on active filters
- 30 ranked story cards, each showing: numbered badge, source icon + name, linked title, TL;DR paragraph, "💡 Why it matters" callout, tag chips, publication date, "Read full story →" link
